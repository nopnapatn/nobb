...
export default class CommandHandler extends Event {
  ...
  Execute(interaction: ChatInputCommandInteraction) {
    ...
    if (timestamps.has(interaction.user.id) && (now < (timestamps.get(interaction.user.id) || 0) + cooldownAmout))
      return interaction.reply({ embeds: [new EmbedBuilder()
        .setColor("Red")
        .setDescription(`❌ Please wait another \`${((((timestamps.get(interaction.user.id) || 0) + cooldownAmout) - now) / 1000).toFixed(1)}\` seconds to run this command!`)
    ], ephemeral: true })
